...
export default function DashboardPage() {
  ...
  useEffect(() => {
    ...
    if (token) {
      try {
        // Assuming JWT format: header.payload.signature
        const payload = token.split(".")[1];
        const decoded = JSON.parse(atob(payload));
        userData = JSON.stringify(decoded.user || decoded);
    ...
    fetch("http://localhost:8080/api/user-dashboard", {
      headers: {
    ...
    fetch("http://localhost:8080/api/users/enrolled-courses", {
      headers: {
        Authorization: `Bearer ${localStorage.getItem("token")}`,
      },
    })
  ...
  //     ? (courseCompletedLessons / courseLessons) * 100
  //     : 0;
  // };
      ...
      <div className="container mx-auto px-4 py-6 md:py-8">
        ...
        <div className="flex items-center space-x-4 mb-6 md:mb-8">
          ...
          <div>
            ...
            {user.role === "admin" && (
              <Badge className="mt-1 text-xs">Admin</Badge>
            )}
          </div>
        </div>
          ...
          {/* <TabsContent value="progress" className="space-y-6">
            ...
            <ProgressTracker
              ...
              onUpdateProgress={(lessonId, timeSpent) => {
                ...
                console.log(
                  `Updated progress for lesson ${lessonId}: ${timeSpent} minutes`
                );
              }}
            />
          </TabsContent> */}
              ...
              <CardContent className="space-y-4">
                <div className="grid md:grid-cols-2 gap-4">
                  ...
                  <div>
                    ...
                    <p className="text-sm md:text-base text-foreground capitalize">
                      {user.role}
                    </p>
                  </div>
                  <div>
