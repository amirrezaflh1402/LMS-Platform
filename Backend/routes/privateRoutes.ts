...
const SECRET_KEY = process.env.JWT_SECRET || "F727E4";
const router = Router();
  ...
  async (req: Request & { user?: JwtPayload }, res: Response) => {
    try {
      ...
      const averageEnrollment = enrollmentsPerCourse.length
        ? enrollmentsPerCourse.reduce((acc, curr) => acc + curr.count, 0) /
          enrollmentsPerCourse.length
        : 0;
    ...
    try {
      ...
      const userDetails = await Promise.all(
        users.map(async (user: any) => {
          ...
          const completedLessonsCount = user.completedLessons
      ...
      res.json({ totalAdmins, totalUsers, users: userDetails });
    } catch (error: any) {
      res
        .status(500)
        .json({ message: "Internal server error", error: error.message });
...
router.get(
  ...
  async (req: Request & { user?: JwtPayload }, res: Response) => {
    try {
      ...
      // Aggregate students count per course
      const studentsCountAgg = await Enrollment.aggregate([
        { $group: { _id: "$course", studentsCount: { $sum: 1 } } },
      ]);
      const studentsCountMap = new Map<string, number>();
      studentsCountAgg.forEach((item: any) => {
        studentsCountMap.set(item._id.toString(), item.studentsCount);
      });

      // Get all courses with instructor, level, price
      const courses = await Course.find()
        .populate("instructor", "name email")
        .select("title instructor level price");
      ...
      const courseList = courses.map((course: any) => ({
        ...
        level: course.level,
        price: course.price,
        studentsCount: studentsCountMap.get(course._id.toString()) || 0,
      }));

      res.json({ courseList: courseList });
    } catch (error: any) {
      res
        .status(500)
        .json({ message: "Internal server error", error: error.message });
